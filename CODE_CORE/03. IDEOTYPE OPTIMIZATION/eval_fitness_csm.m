...
TCLDF = 15.0;
    ...
    for i=1:size(Pop,1)
        if Pop(i).fitness == 0
            ...
            GEN.G5=G5;
            tic
            [MCM_Table] = CERES_Rice(GEN,125);toc  % ENVIRONMENT 1-2-3-4 could be ID"381-423-125-343", related to Fig 2. Environmental categories in the Casamance and Eastern Senegal region"

            Pop(i).phenotype = MCM_Table;   %% Fitness Function
            Pop(i).fitness  = mean( (MCM_Table.HI)+(MCM_Table.WUE*0.1) ) ;    
