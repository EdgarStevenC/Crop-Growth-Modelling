function [] = sensitivity(N, RANDp, rep_num)
%% ***********************************************************************************************************************
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Crop Parameters
...
for i = 1 : N
    ...
    save(strcat('GYsim_rep', num2str(rep_num), '_i', num2str(i), '.mat'), ...
        'GYdb1','GYdb2','GYdb3','GYdb4','GYdb5','GYdb6','GYdb7','GYdb8','GYdb9','GYdb10','GYdb11','GYdb12')

    % OPCIÓN B: Read simulations
    % load(strcat('GYsim_rep', num2str(rep_num), '_i', num2str(i), '.mat'))
...
save(sprintf('senT_rep%d.mat', rep_num), 'GYdb1', 'dP1', 'dP2O', 'dP2R', 'dP5', 'dG1', 'dG2', 'dG3', 'dPHINT', 'dTHOT', 'dTCLDP', 'dTCLDF', ...
              'uP1', 'uP2O', 'uP2R', 'uP5', 'uG1', 'uG2', 'uG3', 'uPHINT', 'uTHOT', 'uTCLDP', 'uTCLDF');
...
load('senT2.mat')
 ...
 %% Grain Yield
    ...
    if vis == 1, TD = SENgraph2(var,unit, STATS, max(STATS(1,:))*1.1); end
 ...
 %% Biomass
  ...
  %% Anthesiif vis == 1,s
    ...
    STATS=[mean(uP1(j,:)),mean(uP2O(j,:)),mean(uP2R(j,:)),mean(uP5(j,:)),mean(uG1(j,:)),mean(uG2(j,:)),mean(uG3(j,:)),mean(uPHINT(j,:)),mean(uTHOT(j,:)),mean(uTCLDP(j,:)),mean(uTCLDF(j,:)); zeros(1,11); 
           mean(dP1(j,:)),mean(dP2O(j,:)),mean(dP2R(j,:)),mean(dP5(j,:)),mean(dG1(j,:)),mean(dG2(j,:)),mean(dG3(j,:)),mean(dPHINT(j,:)),mean(dTHOT(j,:)),mean(dTCLDP(j,:)),mean(dTCLDF(j,:))]
    if vis == 1, TD = SENgraph2(var,unit, STATS, mean(STATS(1,:))*1.1); end
