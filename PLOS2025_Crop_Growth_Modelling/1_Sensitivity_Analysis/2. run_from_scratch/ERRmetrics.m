function [T] = ERRmetrics(DBg, GYdb, BSSdb, ANTdb, MATdb, NGdb, NTdb, ii)
f3 = figure,
MT = [];
%% PHENOLOGY
...
ERR = RMSE;
subplot(1,5,1), plot(GTg, SIMg, "b*"), hold on,    plot(0, 0, "r*"), 
...
[MAE, RMSE, y_mean, NRMSE, R2 ] = METRIC(GTg, SIMg); MT = [MT; [MAE, RMSE, y_mean, NRMSE, R2 ]];
ERR = RMSE;
subplot(1,5,2), plot(GTg, SIMg, "b*"),  hold on,
...
[MAE, RMSE, y_mean, NRMSE, R2 ] = METRIC(GTg, SIMg); MT = [MT; [MAE, RMSE, y_mean, NRMSE, R2 ]];
ERR = RMSE;
subplot(1,5,3), plot(GTg, SIMg, "b*"),  hold on,
 for k = 1 : numel(GTg), text(GTg(k), SIMg(k),  GTgN{k}); end
plot([min([GTg;SIMg])*0.7, max([GTg;SIMg])*1.30],[min([GTg;SIMg])*0.7, max([GTg;SIMg])*1.30], 'k-'),
 grid on, xlabel('Ground Truth'), ylabel('DSSAT simulation'),
...
[MAE, RMSE, y_mean, NRMSE, R2 ] = METRIC(GTg, SIMg); MT = [MT; [MAE, RMSE, y_mean, NRMSE, R2 ]];
ERR = RMSE;
subplot(1,5,4), plot(GTg, SIMg, "b*"),  hold on,
 for k = 1 : numel(GTg), text(GTg(k), SIMg(k),  GTgN{k}); end
plot([min([GTg;SIMg])*0.7, max([GTg;SIMg])*1.30],[min([GTg;SIMg])*0.7, max([GTg;SIMg])*1.30], 'k-'),
 grid on, xlabel('Ground Truth'), ylabel('DSSAT simulation'),
...
subplot(1,5,5), plot(GTg, SIMg, "b*"),  hold on,
 for k = 1 : numel(GTg), text(GTg(k), SIMg(k),  GTgN{k}); end
plot([min([GTg;SIMg])*0.7, max([GTg;SIMg])*1.30],[min([GTg;SIMg])*0.7, max([GTg;SIMg])*1.30], 'k-'),
 grid on, xlabel('Ground Truth'), ylabel('DSSAT simulation'),
...
f3.Position = [ -206.2    342.6    1912.0    366.40];
